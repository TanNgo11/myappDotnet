...
import { CategoryStatus, UpdateCategoryRequest, updateCategorySchema } from '../../../models/Category';
...
const EditCategoryModal: React.FC<EditCategoryModalProps> = ({ isShow, onHide, onCategoryUpdate, categoryId }) => {
    const showToast = useCustomToast();
    const [editCategory, setEditCategory] = React.useState<Category | null>(null);
    const showToastMessage = useCustomToast();
    const [categoryStatus, setCategoryStatus] = useState<CategoryStatus[]>([CategoryStatus.ACTIVE, CategoryStatus.INACTIVE]);
    const { register, handleSubmit, formState: { errors }, reset, setValue } = useForm<UpdateCategoryRequest>({
        mode: "onBlur",
            ...
            <Modal.Body>
                <Form onSubmit={handleSubmit(onSubmit)}>
                    ...
                    <div className="mb-3">
                        ...
                        {errors.status && <div className="text-danger small">{errors.status.message}</div>}
                    </div>
