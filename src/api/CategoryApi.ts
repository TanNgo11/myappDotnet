...
export const updateCategory = async (updateCategory: UpdateCategoryRequest): Promise<ResponseData1<Category>> => {
    try {
        const response = await privateApi.put<ResponseData1<Category>>(`/api/v1/categories`, updateCategory);
        if (response.data.code !== 1000) throw new Error("Error");
...
export const getCategoryByName = async (categoryName: string): Promise<ResponseData1<Category>> => {
    try {
        const response = await privateApi.get<ResponseData1<Category>>(`/api/v1/categories/${categoryName}`);
        if (response.data.code !== 1000) throw new Error("Error");
...
export const getCategoryById = async (categoryId: number): Promise<ResponseData1<Category>> => {
    try {
        const response = await privateApi.get<ResponseData1<Category>>(`/api/v1/categories/${categoryId}`);
        if (response.data.code !== 1000) throw new Error("Error");
...
export const deleteCategories = async (ids: number[]): Promise<ResponseData1<String>> => {
    try {
        const response = await privateApi.delete<ResponseData1<String>>(`/api/v1/categories`, {
            params: { ids },
